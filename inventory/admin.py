...
class ItemAdmin(admin.ModelAdmin):
    list_display = ('name', 'unit', 'is_active', 'created_at', 'updated_at')
    search_fields = ('name',)
    list_filter = ('is_active',)

@admin.register(Inventory)
class InventoryAdmin(admin.ModelAdmin):
    list_display = ('item', 'location', 'quantity', 'created_at', 'updated_at')
    search_fields = ('item__name', 'location__name')
    list_filter = ('location',)

@admin.register(Inward)
class InwardAdmin(admin.ModelAdmin):
    list_display = ('item', 'location', 'quantity', 'received_by', 'date', 'created_at', 'updated_at')
...
class OutwardAdmin(admin.ModelAdmin):
    list_display = ('item', 'location', 'quantity', 'dispatched_by', 'date', 'created_at', 'updated_at')
    search_fields = ('item__name', 'location__name', 'dispatched_by__username')
    list_filter = ('location', 'dispatched_by')
    
@admin.register(InventoryLog)
class InventoryLogAdmin(admin.ModelAdmin):
    list_display = ('item', 'location', 'quantity', 'movement_type', 'reference_id', 'created_at')
